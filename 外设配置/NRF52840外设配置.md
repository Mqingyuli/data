	...
	for (;;)
	{
		...
		if (pdTRUE == xQueueReceive(global.queue_SX1268_irq_msg, &msg, portMAX_DELAY))
		{
			...
			if ((sx1268_irq_status & SX126X_IRQ_RX_DONE) == SX126X_IRQ_RX_DONE)
			{
                ...
                //最后一个数据包的接收信号强度指示和信噪比
				NRF_LOG_INFO("SX1280_1_RSSI:~%d$  SX1280_1_SNR:~%d$",
                     sx1280_1.packet_status.Params.LoRa.RssiPkt,
                     sx1280_1.packet_status.Params.LoRa.SnrPkt);
                //数据长度清零
				payload_len = 0;
                //数组清零
...
```c
static bool lte_is_run(void)//低电平为开机：1； 高电平为关机：0；
{
  uint32_t ret;
